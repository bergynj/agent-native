...
<Endpoint id="doc-block-1qz0910" title={"MCP 端點"} summary={"每個 Agent-Native 應用程式都會公開自動安裝的 Streamable HTTP 端點。"} method="POST" path="/_agent-native/mcp" summary={"MCP 可流式 HTTP 端點"} auth="Standard remote MCP OAuth (Bearer access token), connect-minted JWT, or static ACCESS_TOKEN/ACCESS_TOKENS" params={[
  {
    "name": "Authorization",
    "in": "header",
    "type": "string",
...
代理執行與互動式聊天相同的迴圈 - 它可以呼叫多個工具、推理脈絡並生成深思熟慮的回應。
...
| `/_agent-native/mcp/oauth/token`          | 授權程式碼和重新整理權杖授予 |
