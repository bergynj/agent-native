---
title: "伺服器"
description: "Nitro 伺服器路由、外掛、框架安裝的路由、請求脈絡和 SQL 支援的同步。"
---

# 伺服器

Agent-Native 應用程式使用 [Nitro](https://nitro.build) 作為伺服器路由和外掛。大多數產品行為應該存在於 [Actions](/docs/actions) 中；自訂路由適用於 actions 不適合的協議表面：上傳、流式傳輸、公開頁面、webhooks、OAuth 回呼和特定於提供者的 API。

<Diagram id="doc-block-1gk1inb" title={"伺服器上執行什麼"} summary={"操作是預設的。自訂檔案路由和框架安裝的路由共用相同的 Nitro 應用程式和相同的 SQL 資料庫。"}>

```html
<div class="diagram-server">
  <div class="diagram-col entry">
    <div class="diagram-node">瀏覽器 / UI</div>
    <div class="diagram-node">代理迴圈</div>
    <div class="diagram-node">
      外部用戶端<br /><small class="diagram-muted">HTTP · MCP · A2A</small>
    </div>
  </div>
  ...
  <div class="diagram-panel" data-rough>
    ...
    <div class="diagram-row">
      ...
      ><small class="diagram-muted">預設表面</small>
    </div>
    <div class="diagram-row">
    ...
    <div class="diagram-row">
      ...
      ><small class="diagram-muted">啟動：遷移、工作</small>
    </div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    SQL 資料庫<br /><small class="diagram-muted">Drizzle · 協調點</small>
  </div>
</div>
```
...
`server/routes/` 和 Nitro 中的路由將檔名對應到方法和路徑：
...
- 代理工具。
- 型別化前端掛鉤。
- `/_agent-native/actions/:name`下的 HTTP 端點。
- MCP 和 A2A 可呼叫工具。
- CLI 用於開發的指令。
...
`completeText()` 透過與代理相同的設定引擎層執行
聊天，包括 Builder、Anthropic、AI SDK 提供者、使用者/應用模型預設值，
請求範圍的秘密和引擎規範化的錯誤。它僅適用於伺服器；不要
從用戶端程式碼呼叫模型提供者。如果操作是面向使用者的，則將其包裝起來
在一個操作中，以便 UI 和代理共用相同的功能。

## 請求脈絡和存取 {#request-context}

Actions 由框架自動安裝並與請求脈絡一起執行。自訂路由則不然。如果自訂路由讀取或寫入可擁有的資源，則載入工作階段並包裝工作：

<AnnotatedCode
  id="doc-block-6bqbwo"
  title={"將自訂路由範圍限定為請求使用者"}
...
`getDb`是透過`server/db/index.ts`中的`createGetDb(schema)`為每個應用程式建立的，因此自訂路由從範本（`../../db/index.js`）匯入，而不是從`@agent-native/core/db`匯入；見[Database — Where the DB Client Lives](/docs/database#db-client)。不要在自訂路由中執行無作用域的 `db.select().from(ownableTable)`。

## 伺服器外掛 {#server-plugins}

外掛位於 `server/plugins/` 中並在啟動時執行。將它們用於遷移、提供者設定、重複作業、整合轉接器和框架外掛設定。
...
| `/_agent-native/agent-chat`      | 代理聊天迴圈                                                     |
| `/_agent-native/poll`            | SQL 支援的 UI 同步                                               |
...
Agent-Native 不依賴於檔案系統觀察程式或置頂記憶狀態。當 actions 或框架助手更改資料時，資料庫同步版本會遞增。用戶端 `useDbSync()` 掛鉤輪詢 `/_agent-native/poll` 並使 React 查詢快取無效。

這適用於無伺服器和多執行個體部署，因為資料庫是協調點。如果您在 actions 之外編寫自訂突變，請使用框架助手或發出適當的同步失效，以便開啟 UI 重新整理。

<Diagram id="doc-block-mzpzck" title={"SQL-backed 同步迴圈"} summary={"沒有觀察者，沒有置頂狀態。寫入會碰撞 SQL 中的版本；每個用戶端都會輪詢版本並重新取得。"}>

```html
<div class="diagram-sync">
  <div class="diagram-box" data-rough>
    Action / 輔助函式<br /><small class="diagram-muted">mutates data</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel" data-rough>
...
<Endpoint id="doc-block-1l5m2yi" title={"輪詢端點"} method="GET" path="/_agent-native/poll" summary={"返回目前的每個來源資料庫同步版本，以便用戶端可以偵測更改。"} auth="Session cookie (request-scoped identity)" responses={[
  {
    "status": "200",
...
3. 自觸發簽名的處理器路由。
4. 立即返回 200。
5. 讓新的處理器執行執行代理迴圈並發布結果。

<Diagram id="doc-block-1qv0b1p" title={"整合佇列模式"} summary={"Webhook 處理常式以毫秒為單位返回；單獨的簽名執行執行緩慢的代理工作。"}>

```html
<div class="diagram-webhook">
  <div class="diagram-box" data-rough>
    入站 webhook<br /><small class="diagram-muted"
  ...
  <div class="diagram-panel" data-rough>
    ...
    <div class="diagram-step">
      ...
      ><small class="diagram-muted">將工作寫入 SQL</small>
    </div>
    <div class="diagram-step">
...
<Callout tone="warning">
  不要依賴返回回應後未等待的承諾 — 無伺服器主機會凍結執行。請參閱
  [Messaging](/docs/messaging) 以了解規範整合佇列。
</Callout>

## 進階：逃生艙口 {#advanced-escape-hatches}

大多數範本永遠不需要這些。 Nitro 檔案路由和框架的代理
聊天外掛已經連線應用伺服器和正式環境代理處理常式。
僅在外部建置自訂伺服器整合時才使用它們
標準範本外掛堆堆疊。

### 程式化式 H3 伺服器 {#create-server}
...
### 正式環境代理處理常式 {#agent-handler}

框架的代理聊天外掛已安裝正式環境代理處理常式
用於範本。僅在建置時直接呼叫`createProductionAgentHandler()`
標準範本外掛堆堆疊之外的自訂伺服器整合 -
否則透過`AGENTS.md`、skills、actions和
代理聊天外掛。
...
- [**Actions**](/docs/actions) — 預設操作介面；只有在不適用時才使用自訂路由
- [**資料庫**](/docs/database) — 自訂路由和 actions 都會匯入的 `getDb()`/`schema` 模式
- [**安全性**](/docs/security#access-guards) — `accessFilter`/`assertAccess` 和完整的資料範圍模型
- [**訊息傳遞**](/docs/messaging) — 傳入 webhook 的標準整合佇列模式
- [**MCP 通訊協定**](/docs/mcp-protocol) — 此伺服器自動掛載的 `/_agent-native/mcp` 端點
