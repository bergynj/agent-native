...
本頁將 skill 描述為 `.agents/skills/<name>/SKILL.md` 中的檔案。[Workspace](/docs/workspace#skills) 則將同一個 skill 描述為路徑為 `skills/<slug>/SKILL.md`、範圍為 workspace/shared/personal 的 SQL 資源。兩者是從不同角度描述同一概念：Code 模式直接讀寫檔案；App 模式將其儲存為資源列，因此無需檔案系統也能運作。無論採用哪種方式，都請依本頁格式編寫內容。
...
- [**Workspace**](/docs/workspace#skills) — App 模式下 skills 作為 SQL 資源儲存的位置
