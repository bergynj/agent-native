...
- `databaseTools: "read"` — 預設。僅註冊 `db-schema` 和 `db-query`；
  代理使用 SQL 檢查資料，但必須使用型別化應用程式 actions 進行寫入。
- `databaseTools: "write"` 或 `true` — 也會註冊 `db-exec` 和 `db-patch`
...
[Builder.io](https://www.builder.io) 透過提供託管雲端環境來解決此問題，在該環境中，代理保留在正式環境中修改應用程式的 UI 的能力。將您的儲存庫連線到 Builder.io 並直接提示 UI 更改 - 無需重新部署。

**工作原理：**

1. 將您的 Agent-Native 儲存庫連線到 Builder.io
