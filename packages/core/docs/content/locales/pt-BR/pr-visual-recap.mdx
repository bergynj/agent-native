...
<Diagram id="doc-block-8iqfz5" title={"O que acontece em cada push de relações públicas"} summary={"Uma comparação limitada alimenta um agente de codificação real, que cria uma recapitulação; o fluxo de trabalho faz uma captura de tela e exibe um comentário fixo."}>

```html
<div class="diagram-recap">
  <div class="diagram-node">
    Push do PR<br /><small class="diagram-muted"
  ...
  <div class="diagram-node">
    Agente de código<br /><small class="diagram-muted"
      >Claude Code / Codex lê o diff</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  ...
  <div class="diagram-box">
    Um comentário fixo no PR<br /><small class="diagram-muted"
...
Orientação `visual-recap` em `@agent-native/recap-cli@latest`, incluindo qualquer irmão
...
- **`VISUAL_RECAP_MODEL`** fixa o modelo passado para CLI (`--model`) — por exemplo, `gpt-5.6-luna` para Codex, um ID de modelo Claude, ou `deepseek-chat` para DeepSeek. É obrigatória para `openai-compatible`. Para o back-end **Claude**, se a variável não estiver definida, o padrão agora é `claude-sonnet-5`; defina-a explicitamente para usar um modelo Claude diferente, como o mais barato `claude-haiku-4-5`. Para o back-end **Codex**, se não estiver definida, é usado o modelo padrão do próprio CLI do Codex.
...
Por exemplo, para executar a recapitulação em Codex com GPT-5.6 Sol com raciocínio alto, defina as variáveis de repositório `VISUAL_RECAP_AGENT=codex`, `VISUAL_RECAP_MODEL=gpt-5.6-sol` e `VISUAL_RECAP_REASONING=high`.

## Custo

Cada execução de recapitulação é uma sessão completa de agente de codificação lendo um diff real, não um resumo barato baseado em modelo, então seu custo acompanha a escolha do modelo e a frequência com que é executada:

- **Uma sessão completa por push no PR.** O gatilho padrão dispara em `synchronize`, então um PR que recebe dez pushes executa o agente dez vezes.
- **O PR inteiro a cada vez**, não apenas os commits desde a última execução — o custo escala com o tamanho total do PR, não com o tamanho incremental do diff.
- **Entrada do diff limitada a ~600KB.** Isso limita o pior caso por execução, mas diffs grandes ainda custam significativamente mais do que os pequenos.
- **Uma nova tentativa automática em caso de falha**, o que pode aproximadamente dobrar o custo de uma execução que falha de forma transitória.
...
arquivo de token de publicação que `npx @agent-native/recap-cli@latest recap setup` pode ler:
...
| `VISUAL_RECAP_MODEL`     | `claude-sonnet-5` para Claude; obrigatório para o back-end compatível | Variável. ID do modelo do provedor para back-ends compatíveis com OpenAI (obrigatório nesse caso). Para Claude, se não definida, o padrão agora é `claude-sonnet-5` — defina-a para substituir, por ex. `claude-haiku-4-5` para o nível mais barato. Para Codex, se não definida, usa o próprio padrão do CLI do Codex. |
...
| `RECAP_CLI_VERSION`      | `latest`                                                              | Variável. Fixa a versão `@agent-native/recap-cli` CLI que o fluxo de trabalho instala - por exemplo. `1.5.0`. Consulte [Version pinning](#version-pinning-copy-variant).                                                                                                                                                |
...
O fluxo de trabalho detecta automaticamente como invocar seu auxiliar CLI (fonte local dentro deste monorepo, o `@agent-native/recap-cli` publicado em outro lugar), portanto, não há variável `RECAP_CLI` a ser definida.
...
<Diagram id="doc-block-t1ajfw" title={"Portão de consentimento de RP do Fork"} summary={"Os PRs do Fork não recebem segredos por padrão; autores confiáveis ​​são executados automaticamente e colaboradores externos exigem um novo rótulo de recapitulação do mantenedor."}>
...
<div class="diagram-fork">
  ...
  <div class="diagram-card">
    <span class="diagram-pill warn">Contribuidor externo</span
    ><small class="diagram-muted"
      >mantenedor revisa o diff e depois aplica <code>recap</code></small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center">
    Verificações de gate<br /><small class="diagram-muted"
      >fork PR? &amp; trusted or fresh label?</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box ok">
    Execuções de recapitulação<br /><small class="diagram-muted"
      >apenas código do repo base · diff do fork é entrada de texto</small
    >
  </div>
</div>
```
...
Por padrão, o fluxo de trabalho da variante de cópia instala o `@agent-native/recap-cli@latest` em tempo de execução para que cada execução de recapitulação selecione automaticamente o CLI mais recente. Se seu CI precisar de ferramentas reproduzíveis, defina a variável de repositório **`RECAP_CLI_VERSION`** para fixar a versão instalada:
...
# Substring literal — qualquer linha diff contendo esta string é permitida.
...
Isso é o que `npx @agent-native/recap-cli@latest recap setup --reusable` escreve (ou você pode colá-lo manualmente):

```yaml
name: PR Visual Recap

# Chamador thin — a lógica completa do fluxo de trabalho reside em BuilderIO/agent-native.
# Correções e melhorias chegam a este repositório automaticamente a cada execução.
# Para fixar uma versão específica para reprodutibilidade, substitua '@main' por uma
# tag ou SHA, por exemplo '@v1.2.3' ou '@abc1234'.
...
```bash
# Escreva o chamador thin em vez da cópia completa:
npx @agent-native/recap-cli@latest recap setup --reusable

# Ou com uma referência fixada para reprodutibilidade:
...
Depois de escrever, execute `npx @agent-native/recap-cli@latest recap doctor` normalmente para confirmar se os segredos estão configurados.
...
A entrada `cli-version` controla qual versão do `@agent-native/recap-cli` CLI é executada dentro do fluxo de trabalho. Deixe-a em `"latest"` para rastrear a versão mais recente ou fixe-a em uma string de versão (por exemplo, `"1.5.0"`) para reprodutibilidade total.
