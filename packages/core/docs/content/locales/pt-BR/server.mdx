...
<Diagram id="doc-block-24p8z5" title="O que roda no servidor" summary={"As ações são o padrão. Rotas de arquivo personalizadas e rotas montadas em estrutura compartilham o mesmo aplicativo Nitro e o mesmo banco de dados SQL."}>
...
<div class="diagram-server">
  ...
  <div class="diagram-panel" data-rough>
    ...
    <div class="diagram-row">
      ...
      ><small class="diagram-muted">superfície padrão</small>
    </div>
    <div class="diagram-row">
    ...
    <div class="diagram-row">
      ...
      ><small class="diagram-muted">inicialização: migrações, jobs</small>
    </div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    banco de dados SQL<br /><small class="diagram-muted"
      >Drizzle · ponto de coordenação</small
    >
  </div>
</div>
```
...
<AnnotatedCode
  id="doc-block-x5gayj"
  title={"Escopo de uma rota personalizada para o usuário da solicitação"}
...
<Diagram id="doc-block-11gt0y2" title={"Ciclo de sincronização SQL-backed"} summary={"Sem observadores, sem estado pegajoso. Uma gravação ultrapassa uma versão em SQL; cada cliente pesquisa a versão e busca novamente."}>

```html
<div class="diagram-sync">
  <div class="diagram-box" data-rough>
    Ação / helper<br /><small class="diagram-muted">mutates data</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel" data-rough>
...
<Endpoint id="doc-block-1gyd4tg" title="O ponto final da pesquisa" method="GET" path="/_agent-native/poll" summary={"Retorna as versões atuais de sincronização do banco de dados por origem para que o cliente possa detectar alterações."} auth="Session cookie (request-scoped identity)" responses={[
  {
    "status": "200",
...
<Diagram id="doc-block-1u6y3pu" title={"Padrão de fila de integração"} summary={"O manipulador de webhook retorna em milissegundos; uma execução assinada separada executa o trabalho lento do agente."}>

```html
<div class="diagram-webhook">
  <div class="diagram-box" data-rough>
    Webhook de entrada<br /><small class="diagram-muted"
  ...
  <div class="diagram-panel" data-rough>
    ...
    <div class="diagram-step">
      ...
      ><small class="diagram-muted">insere trabalho no SQL</small>
    </div>
    <div class="diagram-step">
  ...
  <div class="diagram-box" data-rough>
    Signed processor<br /><small class="diagram-muted"
      >executa o loop do agente e publica o resultado</small
    >
  </div>
</div>
```
...
<Callout tone="warning">
  Não confie em promessas inesperadas após retornar uma resposta — hosts sem
  servidor congelam a execução. Consulte [Messaging](/docs/messaging) para ver a
  fila de integração canônica.
</Callout>
...
- [**Actions**](/docs/actions) — a superfície de operação padrão; use uma rota personalizada apenas quando ela não se encaixar
- [**Banco de dados**](/docs/database) — o padrão `getDb()`/`schema` importado por rotas personalizadas e actions
- [**Segurança**](/docs/security#access-guards) — `accessFilter`/`assertAccess` e o modelo completo de escopo de dados
- [**Mensagens**](/docs/messaging) — o padrão canônico de fila de integração para webhooks recebidos
- [**Protocolo MCP**](/docs/mcp-protocol) — o endpoint `/_agent-native/mcp` que este servidor monta automaticamente
