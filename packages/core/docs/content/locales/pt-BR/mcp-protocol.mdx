...
<Diagram id="doc-block-1tzzedn" title="Seu aplicativo como um servidor MCP" summary={"Hosts externos se conectam por Streamable HTTP. Cada ação é uma ferramenta; ask-agent delega para o loop completo do agente."}>
...
<Endpoint id="doc-block-1vujp7k" title="MCP ponto final" summary={"O endpoint Streamable HTTP montado automaticamente que todo aplicativo nativo do agente expõe."} method="POST" path="/_agent-native/mcp" summary={"MCP Endpoint HTTP transmitível"} auth="Standard remote MCP OAuth (Bearer access token), connect-minted JWT, or static ACCESS_TOKEN/ACCESS_TOKENS" params={[
  {
    "name": "Authorization",
    "in": "header",
    "type": "string",
...
<Diagram id="doc-block-yoa2u7" title="Fluxo de descoberta OAuth" summary={"Um 401 inicia a descoberta, o registro e uma autorização PKCE → troca de tokens. O token Bearer é vinculado ao público e tem escopo definido."}>
...
<div class="diagram-oauth">
  ...
  <div class="diagram-box" data-rough>
    Bearer access token<br /><small class="diagram-muted"
      >vinculado à audiência · mcp:read / write / apps</small
    >
  </div>
</div>
```
