...
<Diagram id="doc-block-14afcsz" title={"सर्वर पर क्या चलता है"} summary={"क्रियाएँ डिफ़ॉल्ट हैं. कस्टम फ़ाइल रूट और फ़्रेमवर्क-माउंटेड रूट समान Nitro ऐप और समान SQL डेटाबेस साझा करते हैं।"}>
...
<div class="diagram-server">
  <div class="diagram-col entry">
    ...
    <div class="diagram-node">
      बाहरी क्लाइंट<br /><small class="diagram-muted">HTTP · MCP · A2A</small>
    </div>
  </div>
  ...
  <div class="diagram-panel" data-rough>
    ...
    <div class="diagram-row">
      ...
      ><small class="diagram-muted">डिफ़ॉल्ट सतह</small>
    </div>
    <div class="diagram-row">
    ...
    <div class="diagram-row">
      ...
      ><small class="diagram-muted">स्टार्टअप: माइग्रेशन, jobs</small>
    </div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    SQL डेटाबेस<br /><small class="diagram-muted">Drizzle · समन्वय बिंदु</small>
  </div>
</div>
```
...
<AnnotatedCode
  id="doc-block-2lwds8"
  title={"अनुरोध उपयोगकर्ता के लिए एक कस्टम मार्ग का दायरा"}
...
<Diagram id="doc-block-kuna9v" title={"SQL-backed सिंक लूप"} summary={"कोई देखने वाला नहीं, कोई चिपचिपी स्थिति नहीं। एक लेखन SQL में एक संस्करण को टक्कर देता है; प्रत्येक ग्राहक संस्करण का चुनाव करता है और उसे पुनः प्राप्त करता है।"}>

```html
<div class="diagram-sync">
  <div class="diagram-box" data-rough>
    Action / सहायक<br /><small class="diagram-muted">mutates data</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel" data-rough>
...
<Endpoint id="doc-block-bgb57l" title={"मतदान समापन बिंदु"} method="GET" path="/_agent-native/poll" summary={"वर्तमान प्रति-स्रोत डेटाबेस सिंक संस्करण लौटाएं ताकि क्लाइंट परिवर्तनों का पता लगा सके।"} auth="Session cookie (request-scoped identity)" responses={[
  {
    "status": "200",
...
<Diagram id="doc-block-tp7dg" title={"एकीकरण कतार पैटर्न"} summary={"वेबहुक हैंडलर मिलीसेकंड में लौटता है; एक अलग हस्ताक्षरित निष्पादन धीमी गति से एजेंट कार्य चलाता है।"}>

```html
<div class="diagram-webhook">
  <div class="diagram-box" data-rough>
    इनबाउंड webhook<br /><small class="diagram-muted"
  ...
  <div class="diagram-panel" data-rough>
    ...
    <div class="diagram-step">
      ...
      ><small class="diagram-muted">काम को SQL में डालें</small>
    </div>
    <div class="diagram-step">
  ...
  <div class="diagram-box" data-rough>
    Signed processor<br /><small class="diagram-muted"
      >एजेंट लूप चलाता है और परिणाम पोस्ट करता है</small
    >
  </div>
</div>
```
...
<Callout tone="warning">
  प्रतिक्रिया देने के बाद अप्रतीक्षित वादों पर भरोसा न करें - सर्वर रहित होस्ट
  निष्पादन को रोक देते हैं। विहित एकीकरण कतार के लिए
  [Messaging](/docs/messaging) देखें।
</Callout>
...
- [**Actions**](/docs/actions) — डिफ़ॉल्ट ऑपरेशन सतह; कस्टम रूट का उपयोग तभी करें जब यह इसमें फिट न हो
- [**Database**](/docs/database) — `getDb()`/`schema` पैटर्न जिसे कस्टम रूट और actions दोनों आयात करते हैं
- [**Security**](/docs/security#access-guards) — `accessFilter`/`assertAccess` और पूरा डेटा-स्कोपिंग मॉडल
- [**Messaging**](/docs/messaging) — इनबाउंड webhook के लिए मानक इंटीग्रेशन क्यू पैटर्न
- [**MCP Protocol**](/docs/mcp-protocol) — `/_agent-native/mcp` endpoint जिसे यह सर्वर अपने आप माउंट करता है
