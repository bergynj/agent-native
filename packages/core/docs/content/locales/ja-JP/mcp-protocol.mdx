...
<Diagram id="doc-block-wiv0uw" title={"MCP サーバーとしてのアプリ"} summary={"外部ホストは Streamable HTTP 経由で接続します。それぞれのアクションは 1 つのツールです。 ask-agent は完全なエージェント ループに委任します。"}>
...
<Endpoint id="doc-block-14spw6g" title={"MCP エンドポイント"} summary={"すべてのエージェント ネイティブ アプリが公開する、自動マウントされる Streamable HTTP エンドポイント。"} method="POST" path="/_agent-native/mcp" summary={"MCP ストリーミング可能な HTTP エンドポイント"} auth="Standard remote MCP OAuth (Bearer access token), connect-minted JWT, or static ACCESS_TOKEN/ACCESS_TOKENS" params={[
  {
    "name": "Authorization",
    "in": "header",
    "type": "string",
...
<Diagram id="doc-block-isdrqb" title={"OAuth 検出フロー"} summary={"401 によって検出、登録、PKCE による承認→トークン交換が開始されます。 Bearer トークンは、対象者に限定され、スコープが設定されています。"}>
...
<div class="diagram-oauth">
  ...
  <div class="diagram-box" data-rough>
    Bearer access token<br /><small class="diagram-muted"
      >audienceバインド · mcp:read / write / apps</small
    >
  </div>
</div>
```
