...
<Diagram id="doc-block-1b26mt2" title={"サーバー上で実行されるもの"} summary={"アクションはデフォルトです。カスタム ファイル ルートとフレームワークにマウントされたルートは、同じ Nitro アプリと同じ SQL データベースを共有します。"}>

```html
<div class="diagram-server">
  <div class="diagram-col entry">
    <div class="diagram-node">ブラウザー / UI</div>
    <div class="diagram-node">エージェントループ</div>
  ...
  <div class="diagram-panel" data-rough>
    ...
    <div class="diagram-row">
      ...
      ><small class="diagram-muted">デフォルトのサーフェス</small>
    </div>
    <div class="diagram-row">
    ...
    <div class="diagram-row">
      ...
      ><small class="diagram-muted">起動: マイグレーション、ジョブ</small>
    </div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    SQLデータベース<br /><small class="diagram-muted"
      >Drizzle · 調整ポイント</small
    >
  </div>
</div>
```
...
<AnnotatedCode
  id="doc-block-2srqi"
  title={"カスタムルートのスコープをリクエストユーザーに設定する"}
...
<Diagram id="doc-block-oizi04" title={"SQL-backed 同期ループ"} summary={"ウォッチャーもスティッキー状態もありません。書き込みにより SQL のバージョンが上がります。すべてのクライアントがバージョンをポーリングし、再フェッチします。"}>

```html
<div class="diagram-sync">
  <div class="diagram-box" data-rough>
    Action / ヘルパー<br /><small class="diagram-muted">mutates data</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel" data-rough>
    <strong>SQLデータベース</strong
...
<Endpoint id="doc-block-1rmccq6" title={"投票エンドポイント"} method="GET" path="/_agent-native/poll" summary={"クライアントが変更を検出できるように、現在のソースごとのデータベース同期バージョンを返します。"} auth="Session cookie (request-scoped identity)" responses={[
  {
    "status": "200",
...
<Diagram id="doc-block-17fvm" title={"統合キューのパターン"} summary={"Webhook ハンドラーはミリ秒単位で戻ります。別の署名付き実行では、エージェントの作業が遅くなります。"}>

```html
<div class="diagram-webhook">
  <div class="diagram-box" data-rough>
    受信webhook<br /><small class="diagram-muted">Slack · Stripe · email</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel" data-rough>
    ...
    <div class="diagram-step">
      ...
      ><small class="diagram-muted">作業を SQL に挿入</small>
    </div>
    <div class="diagram-step">
...
<Callout tone="warning">
  応答を返した後は、待機していない Promise に依存しないでください。サーバーレス
  ホストは実行をフリーズします。正規統合キューについては、[Messaging](/docs/messaging)
  を参照してください。
</Callout>
...
- [**Actions**](/docs/actions) — デフォルトの操作サーフェス。適合しない場合にのみカスタムルートを使用します
- [**Database**](/docs/database) — カスタムルートと Actions の両方がインポートする `getDb()`/`schema` パターン
- [**Security**](/docs/security#access-guards) — `accessFilter`/`assertAccess` と完全なデータスコープモデル
- [**Messaging**](/docs/messaging) — 受信 Webhook の標準的な統合キューパターン
- [**MCP Protocol**](/docs/mcp-protocol) — このサーバーが自動的にマウントする `/_agent-native/mcp` エンドポイント
