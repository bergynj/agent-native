...
<Diagram id="doc-block-1wdbcth" title={"服务器上运行什么"} summary={"操作是默认的。自定义文件路由和框架安装的路由共享相同的 Nitro 应用程序和相同的 SQL 数据库。"}>

```html
<div class="diagram-server">
  <div class="diagram-col entry">
    <div class="diagram-node">浏览器 / UI</div>
    <div class="diagram-node">代理循环</div>
  ...
  <div class="diagram-panel" data-rough>
    ...
    <div class="diagram-row">
      ...
      ><small class="diagram-muted">默认表面</small>
    </div>
    <div class="diagram-row">
    ...
    <div class="diagram-row">
      ...
      ><small class="diagram-muted">启动：迁移、任务</small>
    </div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    SQL 数据库<br /><small class="diagram-muted">Drizzle · 协调点</small>
  </div>
</div>
```
...
<Diagram id="doc-block-1uo918o" title={"SQL-backed 同步循环"} summary={"没有观察者，没有粘性状态。写入会碰撞 SQL 中的版本；每个客户端都会轮询版本并重新获取。"}>

```html
<div class="diagram-sync">
  <div class="diagram-box" data-rough>
    Action / 辅助函数<br /><small class="diagram-muted">mutates data</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel" data-rough>
...
<Endpoint id="doc-block-w623lk" title={"轮询端点"} method="GET" path="/_agent-native/poll" summary={"返回当前的每个源数据库同步版本，以便客户端可以检测更改。"} auth="Session cookie (request-scoped identity)" responses={[
  {
    "status": "200",
...
<Diagram id="doc-block-vp9bf6" title={"集成队列模式"} summary={"Webhook 处理程序以毫秒为单位返回；单独的签名执行运行缓慢的代理工作。"}>
...
<div class="diagram-webhook">
  ...
  <div class="diagram-panel" data-rough>
    ...
    <div class="diagram-step">
      ...
      ><small class="diagram-muted">将工作写入 SQL</small>
    </div>
    <div class="diagram-step">
...
<Callout tone="warning">
  不要依赖返回响应后未等待的承诺 — 无服务器主机会冻结执行。请参阅
  [Messaging](/docs/messaging) 以了解规范集成队列。
</Callout>
...
- [**Actions**](/docs/actions) — 默认操作表面；仅当操作不适用时才使用自定义路由
- [**数据库**](/docs/database) — 自定义路由和 actions 都会导入的 `getDb()`/`schema` 模式
- [**安全**](/docs/security#access-guards) — `accessFilter`/`assertAccess` 和完整的数据作用域模型
- [**消息传递**](/docs/messaging) — 入站 webhook 的标准集成队列模式
- [**MCP 协议**](/docs/mcp-protocol) — 此服务器自动挂载的 `/_agent-native/mcp` 端点
