...
本页将 skill 描述为 `.agents/skills/<name>/SKILL.md` 中的文件。[Workspace](/docs/workspace#skills) 则将同一个 skill 描述为路径为 `skills/<slug>/SKILL.md`、范围为 workspace/shared/personal 的 SQL 资源。两者是从不同角度描述同一概念：Code 模式直接读写文件；App 模式将其存储为资源行，因此无需文件系统也能工作。无论采用哪种方式，都请按本页格式编写内容。
...
- [**Workspace**](/docs/workspace#skills) — App 模式下 skills 作为 SQL 资源存储的位置
