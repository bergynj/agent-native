...
<Diagram id="doc-block-jea5wu" title={"您的应用程序作为 MCP 服务器"} summary={"外部主机通过 Streamable HTTP 连接。每个动作都是一个工具； Ask-agent 委托给完整的代理循环。"}>
...
<Endpoint id="doc-block-1mrk9b2" title={"MCP端点"} summary={"每个代理本机应用程序都会公开自动安装的 Streamable HTTP 端点。"} method="POST" path="/_agent-native/mcp" summary={"MCP 可流式 HTTP 端点"} auth="Standard remote MCP OAuth (Bearer access token), connect-minted JWT, or static ACCESS_TOKEN/ACCESS_TOKENS" params={[
  {
    "name": "Authorization",
    "in": "header",
    "type": "string",
...
<Diagram id="doc-block-bp55uo" title={"OAuth 发现流程"} summary={"401 启动发现、注册和 PKCE 授权 → 令牌交换。不记名令牌是受受众限制和范围的。"}>
...
<div class="diagram-oauth">
  ...
  <div class="diagram-box" data-rough>
    Bearer access token<br /><small class="diagram-muted"
      >绑定 audience · mcp:read / write / apps</small
    >
  </div>
</div>
```
