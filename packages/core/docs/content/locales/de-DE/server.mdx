...
<Diagram id="doc-block-1x03es6" title={"Was auf dem Server läuft"} summary={"Aktionen sind die Standardeinstellung. Benutzerdefinierte Dateirouten und im Framework bereitgestellte Routen nutzen dieselbe Nitro-App und dieselbe SQL-Datenbank."}>

```html
<div class="diagram-server">
  <div class="diagram-col entry">
    <div class="diagram-node">Browser / Benutzeroberfläche</div>
  ...
  <div class="diagram-panel" data-rough>
    ...
    <div class="diagram-row">
      ...
      ><small class="diagram-muted">Standardoberfläche</small>
    </div>
    <div class="diagram-row">
    ...
    <div class="diagram-row">
      ...
      ><small class="diagram-muted">Start: Migrationen, Jobs</small>
    </div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    SQL-Datenbank<br /><small class="diagram-muted"
      >Drizzle · Koordinationspunkt</small
    >
  </div>
</div>
```
...
<AnnotatedCode
  id="doc-block-uwrkhu"
  title="Festlegen einer benutzerdefinierten Route zum Anforderungsbenutzer"
...
<Diagram id="doc-block-16wa4ma" title="SQL-backed Synchronisierungsschleife" summary={"Keine Beobachter, kein klebriger Zustand. Ein Schreibvorgang stößt eine Version in SQL an; Jeder Client fragt die Version ab und ruft sie erneut ab."}>
...
<Endpoint id="doc-block-la2pz2" title="Der Umfrageendpunkt" method="GET" path="/_agent-native/poll" summary={"Gibt die aktuellen Datenbanksynchronisierungsversionen pro Quelle zurück, damit der Client Änderungen erkennen kann."} auth="Session cookie (request-scoped identity)" responses={[
  {
    "status": "200",
...
<Diagram id="doc-block-y62aic" title="Muster der Integrationswarteschlange" summary={"Der Webhook-Handler gibt in Millisekunden zurück; Eine separate signierte Ausführung führt die Arbeit des langsamen Agenten aus."}>

```html
<div class="diagram-webhook">
  <div class="diagram-box" data-rough>
    Eingehender Webhook<br /><small class="diagram-muted"
  ...
  <div class="diagram-panel" data-rough>
    ...
    <div class="diagram-step">
      ...
      ><small class="diagram-muted">Arbeit in SQL einfügen</small>
    </div>
    <div class="diagram-step">
  ...
  <div class="diagram-box" data-rough>
    Signed processor<br /><small class="diagram-muted"
      >führt die Agentenschleife aus und postet das Ergebnis</small
    >
  </div>
</div>
```
...
<Callout tone="warning">
  Verlassen Sie sich nicht auf unerwartete Versprechen, nachdem Sie eine Antwort
  zurückgegeben haben – serverlose Hosts frieren die Ausführung ein.
  Informationen zur kanonischen Integrationswarteschlange finden Sie unter
...
- [**Actions**](/docs/actions) – die Standardoberfläche für Operationen; verwenden Sie eine benutzerdefinierte Route nur, wenn sie dort nicht passt
- [**Datenbank**](/docs/database) – das `getDb()`/`schema`-Muster, das benutzerdefinierte Routen und Actions importieren
- [**Sicherheit**](/docs/security#access-guards) – `accessFilter`/`assertAccess` und das vollständige Datenscoping-Modell
- [**Messaging**](/docs/messaging) – das kanonische Muster für die Integrationswarteschlange eingehender Webhooks
- [**MCP-Protokoll**](/docs/mcp-protocol) – der `/_agent-native/mcp`-Endpunkt, den dieser Server automatisch einbindet
