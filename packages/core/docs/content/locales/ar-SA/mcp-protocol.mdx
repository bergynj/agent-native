...
<Diagram id="doc-block-1dup1a3" title={"تطبيقك كخادم MCP"} summary={"يتصل المضيفون الخارجيون عبر HTTP القابل للتدفق. كل عمل هو أداة واحدة؛ مندوبو Ask-agent إلى حلقة الوكيل الكاملة."}>
...
<Endpoint id="doc-block-17mmuo8" title={"MCP نقطة النهاية"} summary={"نقطة نهاية Streamable HTTP المثبتة تلقائيًا والتي يكشفها كل تطبيق أصلي للوكيل."} method="POST" path="/_agent-native/mcp" summary={"MCP نقطة النهاية HTTP القابلة للبث"} auth="Standard remote MCP OAuth (Bearer access token), connect-minted JWT, or static ACCESS_TOKEN/ACCESS_TOKENS" params={[
  {
    "name": "Authorization",
    "in": "header",
    "type": "string",
...
<Diagram id="doc-block-7e3ku8" title={"OAuth تدفق الاكتشاف"} summary={"يبدأ 401 عملية الاكتشاف والتسجيل وتفويض PKCE → تبادل الرمز المميز. الرمز المميز لحامله مرتبط بالجمهور ومحدد النطاق."}>
...
<div class="diagram-oauth">
  ...
  <div class="diagram-box" data-rough>
    Bearer access token<br /><small class="diagram-muted"
      >مرتبط بالجمهور · mcp:read / write / apps</small
    >
  </div>
</div>
```
