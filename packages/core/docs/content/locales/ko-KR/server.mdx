...
<Diagram id="doc-block-lj4dzm" title={"서버에서 실행되는 것"} summary={"조치가 기본값입니다. 사용자 정의 파일 경로와 프레임워크 탑재 경로는 동일한 Nitro 앱과 동일한 SQL 데이터베이스를 공유합니다."}>
...
<div class="diagram-server">
  ...
  <div class="diagram-panel" data-rough>
    ...
    <div class="diagram-row">
      ...
      ><small class="diagram-muted">기본 표면</small>
    </div>
    <div class="diagram-row">
    ...
    <div class="diagram-row">
      ...
      ><small class="diagram-muted">시작: 마이그레이션, 작업</small>
    </div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
...
<Diagram id="doc-block-m1gowt" title={"SQL-backed 동기화 루프"} summary={"감시자도 없고 끈끈한 상태도 없습니다. 쓰기는 SQL의 버전과 충돌합니다. 모든 클라이언트는 버전을 폴링하고 다시 가져옵니다."}>

```html
<div class="diagram-sync">
  <div class="diagram-box" data-rough>
    Action / 헬퍼<br /><small class="diagram-muted">mutates data</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel" data-rough>
...
<Endpoint id="doc-block-vqe3d1" title={"설문 조사 끝점"} method="GET" path="/_agent-native/poll" summary={"클라이언트가 변경 사항을 감지할 수 있도록 현재 소스별 데이터베이스 동기화 버전을 반환합니다."} auth="Session cookie (request-scoped identity)" responses={[
  {
    "status": "200",
...
<Diagram id="doc-block-1wnvkdi" title={"통합 대기열 패턴"} summary={"웹훅 핸들러는 밀리초 단위로 반환됩니다. 별도의 서명된 실행은 느린 에이전트 작업을 실행합니다."}>

```html
<div class="diagram-webhook">
  <div class="diagram-box" data-rough>
    인바운드 webhook<br /><small class="diagram-muted"
  ...
  <div class="diagram-panel" data-rough>
    ...
    <div class="diagram-step">
      ...
      ><small class="diagram-muted">작업을 SQL에 삽입</small>
    </div>
    <div class="diagram-step">
...
<Callout tone="warning">
  응답을 반환한 후 기다리지 않은 약속에 의존하지 마십시오. 서버리스 호스트는
  실행을 정지시킵니다. 정식 통합 대기열은 [Messaging](/docs/messaging)를
  참조하세요.
</Callout>
...
- [**Actions**](/docs/actions) — 기본 작업 표면이며, 여기에 맞지 않을 때만 사용자 지정 경로를 사용합니다
- [**Database**](/docs/database) — 사용자 지정 경로와 작업이 모두 가져오는 `getDb()`/`schema` 패턴
- [**Security**](/docs/security#access-guards) — `accessFilter`/`assertAccess`와 전체 데이터 범위 지정 모델
- [**Messaging**](/docs/messaging) — 인바운드 webhook을 위한 표준 통합 큐 패턴
- [**MCP Protocol**](/docs/mcp-protocol) — 이 서버가 자동으로 마운트하는 `/_agent-native/mcp` 엔드포인트
