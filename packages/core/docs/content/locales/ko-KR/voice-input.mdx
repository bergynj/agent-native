...
음성 모델은 채팅과 동일한 인증된 Agent Native 도구를 사용하므로 말하는 동안 앱을 탐색하고 작업을 호출할 수 있습니다. 승인이 필요한 작업은 사용자가 승인할 때까지 일시 중지된 상태로 유지됩니다. 오디오는 SQL에 저장되지 않으며 Agent Native는 간결한 세션 및 대화 내용 상태만 보관합니다. 실시간 세션은 최대 60분 동안 지속됩니다.
