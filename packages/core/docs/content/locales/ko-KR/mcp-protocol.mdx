...
<Diagram id="doc-block-1yvyq4q" title={"MCP 서버로서의 앱"} summary={"외부 호스트는 Streamable HTTP을 통해 연결됩니다. 각 작업은 하나의 도구입니다. Ask-agent는 전체 에이전트 루프에 위임합니다."}>
...
<Endpoint id="doc-block-6peopk" title={"MCP 엔드포인트"} summary={"모든 에이전트 기반 앱이 노출하는 자동 탑재 스트리밍 가능 HTTP 엔드포인트입니다."} method="POST" path="/_agent-native/mcp" summary={"MCP 스트리밍 가능한 HTTP 엔드포인트"} auth="Standard remote MCP OAuth (Bearer access token), connect-minted JWT, or static ACCESS_TOKEN/ACCESS_TOKENS" params={[
  {
    "name": "Authorization",
    "in": "header",
    "type": "string",
...
<Diagram id="doc-block-6e3pyn" title={"OAuth 검색 흐름"} summary={"401은 검색, 등록 및 PKCE 승인 → 토큰 교환을 시작합니다. Bearer 토큰은 대상에 국한되며 범위가 지정됩니다."}>
...
<div class="diagram-oauth">
  ...
  <div class="diagram-box" data-rough>
    Bearer access token<br /><small class="diagram-muted"
      >audience 바인딩 · mcp:read / write / apps</small
    >
  </div>
</div>
```
