...
<Diagram id="doc-block-ut4q1s" title="Votre application en tant que serveur MCP" summary={"Les hôtes externes se connectent via Streamable HTTP. Chaque action est un outil ; Ask-agent délègue à la boucle complète de l'agent."}>
...
<div class="diagram-mcp">
  ...
  <div class="diagram-panel">
    <span class="diagram-pill accent">POST /_agent-native/mcp</span
...
<Endpoint id="doc-block-bd4fya" title="Point de terminaison MCP" summary={"Le point de terminaison Streamable HTTP monté automatiquement et exposé par chaque application native d'agent."} method="POST" path="/_agent-native/mcp" summary="MCP Point de terminaison HTTP diffusable" auth="Standard remote MCP OAuth (Bearer access token), connect-minted JWT, or static ACCESS_TOKEN/ACCESS_TOKENS" params={[
  {
    "name": "Authorization",
    "in": "header",
    "type": "string",
...
<Diagram id="doc-block-l7acbd" title={"Flux de découverte OAuth"} summary={"Un 401 lance la découverte, l'enregistrement et un PKCE autorisation → échange de jetons. Le jeton Bearer est limité au public et limité."}>
...
<div class="diagram-oauth">
  ...
  <div class="diagram-box" data-rough>
    Bearer access token<br /><small class="diagram-muted"
      >lié à l’audience · mcp:read / write / apps</small
    >
  </div>
</div>
```
