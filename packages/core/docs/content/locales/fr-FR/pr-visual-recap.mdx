...
<Diagram id="doc-block-1a4ar0y" title={"Que se passe-t-il à chaque poussée de relations publiques"} summary={"Une différence limitée alimente un véritable agent de codage, qui rédige un récapitulatif ; le flux de travail en fait une capture d'écran et insère un commentaire collant."}>

```html
<div class="diagram-recap">
  <div class="diagram-node">
    Push du PR<br /><small class="diagram-muted"
  ...
  <div class="diagram-node">
    Agent de code<br /><small class="diagram-muted"
      >Claude Code / Codex lit le diff</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  ...
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-node">
    Chrome sans interface<br /><small class="diagram-muted"
      >light + dark screenshots</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box">
    Un commentaire PR persistant<br /><small class="diagram-muted"
...
Cela installe la compétence `visual-plan` (qui inclut la compétence `visual-recap` exécutée par l'action) et écrit `.github/workflows/pr-visual-recap.yml` dans votre dépôt. Le flux de travail appelle **les sous-commandes CLI publiées** via `npx @agent-native/recap-cli@latest recap <subcommand>` — notamment `gate`, `collect-diff`, `block-reference`, `scan`, `build-prompt`, `publish`, `shot`, `comment`, `check` et `usage` — afin que rien ne soit copié dans votre dépôt en tant que scripts d'assistance. `setup` et `doctor` sont les assistants interactifs que vous exécutez localement ; `gate` est l'étape de sécurité que le flux de travail exécute avant chaque récapitulatif.
...
Conseils `visual-recap` dans `@agent-native/recap-cli@latest`, y compris les frères et sœurs
...
- **`VISUAL_RECAP_MODEL`** épingle le modèle transmis au CLI (`--model`) — par exemple `gpt-5.6-luna` pour Codex, un identifiant de modèle Claude, ou `deepseek-chat` pour DeepSeek. Elle est requise pour `openai-compatible`. Pour le backend **Claude**, la laisser non définie utilise désormais par défaut `claude-sonnet-5` ; définissez la variable explicitement pour utiliser un autre modèle Claude, comme le moins cher `claude-haiku-4-5`. Pour le backend **Codex**, la laisser non définie utilise le modèle par défaut du CLI Codex lui-même.
...
Par exemple, pour exécuter le récapitulatif sur Codex avec GPT-5.6 Sol avec un raisonnement élevé, définissez les variables du référentiel `VISUAL_RECAP_AGENT=codex`, `VISUAL_RECAP_MODEL=gpt-5.6-sol` et `VISUAL_RECAP_REASONING=high`.

## Coût

Chaque exécution du récapitulatif est une session complète d'agent de codage agentique qui lit un véritable diff, et non un résumé calqué sur un modèle bon marché ; son coût dépend donc du modèle choisi et de la fréquence d'exécution :

- **Une session complète par poussée de PR.** Le déclencheur par défaut se déclenche sur `synchronize`, donc une PR poussée dix fois exécute l'agent dix fois.
- **La PR entière à chaque fois**, pas seulement les commits depuis la dernière exécution — le coût augmente avec la taille totale de la PR, pas avec la taille incrémentale du diff.
- **Entrée de diff plafonnée à environ 600 Ko.** Cela borne le pire cas par exécution, mais les diffs volumineux coûtent tout de même sensiblement plus cher que les petits.
- **Une nouvelle tentative automatique en cas d'échec**, ce qui peut à peu près doubler le coût d'une exécution qui échoue de manière transitoire.
...
fichier de jeton de publication que `npx @agent-native/recap-cli@latest recap setup` peut lire :
...
| `VISUAL_RECAP_MODEL`     | `claude-sonnet-5` pour Claude ; requis pour le backend compatible | Variable. Identifiant du modèle du fournisseur pour les backends compatibles OpenAI (requis dans ce cas). Pour Claude, non défini utilise désormais par défaut `claude-sonnet-5` — définissez-le pour le remplacer, par ex. `claude-haiku-4-5` pour le niveau le moins cher. Pour Codex, non défini utilise la valeur par défaut du CLI Codex lui-même. |
...
| `RECAP_CLI_VERSION`      | `latest`                                                          | Variable. Épingle la version `@agent-native/recap-cli` CLI installée par le flux de travail, par ex. `1.5.0`. Voir [Version pinning](#version-pinning-copy-variant).                                                                                                                                                                                    |
...
Le workflow détecte automatiquement comment appeler son assistant CLI (source locale dans ce monorepo, le `@agent-native/recap-cli` publié ailleurs), il n'y a donc aucune variable `RECAP_CLI` à définir.
...
<Diagram id="doc-block-sqtdd5" title="Porte de consentement Fork PR" summary={"Les PR de Fork n'ont aucun secret par défaut ; les auteurs de confiance s'exécutent automatiquement et les contributeurs externes nécessitent une nouvelle étiquette récapitulative du responsable."}>
...
<div class="diagram-fork">
  ...
  <div class="diagram-card">
    ...
    ><small class="diagram-muted"
      >le mainteneur examine le diff, puis l’applique <code>recap</code></small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center">
    Vérifications de garde<br /><small class="diagram-muted"
      >fork PR? &amp; trusted or fresh label?</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box ok">
    Exécutions de récapitulatif<br /><small class="diagram-muted"
      >code du repo de base uniquement · le diff du fork est une entrée
      texte</small
    >
  </div>
</div>
...
Par défaut, le workflow de copie de variante installe `@agent-native/recap-cli@latest` au moment de l'exécution afin que chaque exécution de récapitulation récupère automatiquement le dernier CLI. Si votre CI a besoin d'outils reproductibles, définissez la variable du référentiel **`RECAP_CLI_VERSION`** pour épingler la version installée :
...
Avant de publier un récapitulatif, le workflow exécute `npx @agent-native/recap-cli@latest recap scan` pour détecter les secrets probables dans le diff. Tout PR dont la différence correspond à un modèle secret connu est bloqué avec un commentaire explicatif : le récapitulatif n'est pas publié et aucun contenu de différence n'est envoyé à l'agent de codage.
...
# Sous-chaîne littérale — toute ligne de comparaison contenant cette chaîne est autorisée.
...
Voici ce qu'écrit `npx @agent-native/recap-cli@latest recap setup --reusable` (ou vous pouvez le coller manuellement) :

```yaml
name: PR Visual Recap

# Appelant léger : la logique complète du flux de travail réside dans BuilderIO/agent-native.
# Les correctifs et améliorations atteignent ce dépôt automatiquement à chaque exécution.
# Pour épingler une version spécifique à des fins de reproductibilité, remplacez « @main » par un
# tag ou SHA, par ex. '@v1.2.3' ou '@abc1234'.
...
```bash
# Écrivez l'appelant léger au lieu de la copie complète :
npx @agent-native/recap-cli@latest recap setup --reusable

# Ou avec une référence épinglée pour plus de reproductibilité :
...
Après l'écriture, exécutez `npx @agent-native/recap-cli@latest recap doctor` comme d'habitude pour confirmer que les secrets sont configurés.
...
L'entrée `cli-version` contrôle quelle version de `@agent-native/recap-cli` CLI s'exécute dans le flux de travail : laissez-la sur `"latest"` pour suivre la version la plus récente, ou épinglez-la à une chaîne de version (par exemple, `"1.5.0"`) pour une reproductibilité totale.
