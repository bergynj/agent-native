...
<Diagram id="doc-block-1lz7fen" title={"Ce qui s'exécute sur le serveur"} summary={"Les actions sont la valeur par défaut. Les routes de fichiers personnalisées et les routes montées sur le framework partagent la même application Nitro et la même base de données SQL."}>

```html
<div class="diagram-server">
  <div class="diagram-col entry">
    <div class="diagram-node">Navigateur / UI</div>
    <div class="diagram-node">boucle agent</div>
  ...
  <div class="diagram-panel" data-rough>
    ...
    <div class="diagram-row">
      ...
      ><small class="diagram-muted">surface par défaut</small>
    </div>
    <div class="diagram-row">
    ...
    <div class="diagram-row">
      ...
      ><small class="diagram-muted">démarrage : migrations, jobs</small>
    </div>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    base de données SQL<br /><small class="diagram-muted"
      >Drizzle · point de coordination</small
    >
  </div>
</div>
```
...
<AnnotatedCode
  id="doc-block-wrqlry"
  title={
    "Attribution d'un itinéraire personnalisé à l'utilisateur de la demande"
  }
...
<Diagram id="doc-block-1q2lj8f" title="Boucle de synchronisation SQL-backed" summary={"Pas d'observateurs, pas d'état collant. Une écriture modifie une version dans SQL ; chaque client interroge la version et la récupère."}>

```html
<div class="diagram-sync">
  <div class="diagram-box" data-rough>
    Action / assistant<br /><small class="diagram-muted">mutates data</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel" data-rough>
...
<Endpoint id="doc-block-p16bwu" title="Le point final du sondage" method="GET" path="/_agent-native/poll" summary={"Renvoie les versions actuelles de synchronisation de la base de données par source afin que le client puisse détecter les modifications."} auth="Session cookie (request-scoped identity)" responses={[
  {
    "status": "200",
...
<Diagram id="doc-block-1fz9tsb" title={"Modèle de file d'attente d'intégration"} summary={"Le gestionnaire de webhook renvoie en millisecondes ; une exécution signée distincte exécute le travail lent de l'agent."}>

```html
<div class="diagram-webhook">
  <div class="diagram-box" data-rough>
    Webhook entrant<br /><small class="diagram-muted"
  ...
  <div class="diagram-panel" data-rough>
    ...
    <div class="diagram-step">
      ...
      ><small class="diagram-muted">insère le travail dans SQL</small>
    </div>
    <div class="diagram-step">
  ...
  <div class="diagram-box" data-rough>
    Signed processor<br /><small class="diagram-muted"
      >exécute la boucle agent et publie le résultat</small
    >
  </div>
</div>
```
...
<Callout tone="warning">
  Ne vous fiez pas à des promesses non attendues après avoir renvoyé une
  réponse : les hôtes sans serveur gèlent l'exécution. Voir
  [Messaging](/docs/messaging) pour la file d'attente d'intégration canonique.
</Callout>
...
- [**Actions**](/docs/actions) — la surface d'opération par défaut ; n'utilisez une route personnalisée que si elle ne convient pas
- [**Base de données**](/docs/database) — le modèle `getDb()`/`schema` importé par les routes personnalisées et les actions
- [**Sécurité**](/docs/security#access-guards) — `accessFilter`/`assertAccess` et le modèle complet de portée des données
- [**Messagerie**](/docs/messaging) — le modèle canonique de file d'attente d'intégration pour les webhooks entrants
- [**Protocole MCP**](/docs/mcp-protocol) — le point de terminaison `/_agent-native/mcp` que ce serveur monte automatiquement
